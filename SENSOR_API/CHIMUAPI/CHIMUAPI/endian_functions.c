...
{
  unsigned char b1, b2;
  if(gCHIMU_Endian_Is_Small == TRUE) return (s); //Not needed
...
int LongSwap (int i)
{
  ...
  if(gCHIMU_Endian_Is_Small == TRUE) return (i); //Not needed
...
{
  if(gCHIMU_Endian_Is_Small == TRUE) return (f); //Not needed
  union
  {
    float f;
    unsigned char b[4];
